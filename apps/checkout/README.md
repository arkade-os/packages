...
Create a `.env.local` file (automatically generated by `node ./node_modules/@arkade-os/checkout/cli/create.js`):
...
1. Push to GitHub
2. Import to Vercel
3. Set **Root Directory** to the repo root (not `apps/checkout`)
4. Set **Build Command** to:
   ```bash
   pnpm -r --filter @arkade-os/checkout... build
   ```
5. Add environment variables:
   ...
   - `BOLTZ_API_URL`
   - `ARKADE_NETWORK`
6. Optionally configure Vercel KV for storage
7. Deploy!
...
Run `node ./node_modules/@arkade-os/checkout/cli/create.js` to generate `.env.local`
