...
describe('Validation Utilities', () => {
  ...
  describe('validateNetwork', () => {
    ...
    it('throws on non-string network', () => {
      ...
      expect(() => validateNetwork(undefined)).toThrow('Invalid network type: expected string');
    });
  ...
  describe('validateSignerPubkey', () => {
    ...
    it('throws on non-string pubkey', () => {
      expect(() => validateSignerPubkey(123)).toThrow('Invalid signerPubkey type: expected string');
    ...
    it('throws on incorrect length (not 64 chars)', () => {
      expect(() => validateSignerPubkey('a'.repeat(32))).toThrow('expected 64 hex characters');
      expect(() => validateSignerPubkey('a'.repeat(66))).toThrow('expected 64 hex characters');
    });
  ...
  describe('validateInputIndexes', () => {
    ...
    it('throws on non-array', () => {
      ...
      expect(() => validateInputIndexes(123)).toThrow('Invalid inputIndexes type: expected array');
      expect(() => validateInputIndexes(null)).toThrow('Invalid inputIndexes type: expected array');
    });
  ...
  describe('validateUnilateralExitDelay', () => {
    ...
    it('throws on non-numeric types', () => {
      expect(() => validateUnilateralExitDelay(null)).toThrow('Invalid unilateralExitDelay type');
      expect(() => validateUnilateralExitDelay(undefined)).toThrow(
        'Invalid unilateralExitDelay type',
      );
      expect(() => validateUnilateralExitDelay({})).toThrow('Invalid unilateralExitDelay type');
    });

    it('throws on invalid numeric string', () => {
      expect(() => validateUnilateralExitDelay('not-a-number')).toThrow('is not a valid number');
