...
describe('RPC Handler', () => {
  ...
  describe('arkade_getAddress', () => {
    ...
    it('throws on invalid network name', async () => {
      await expect(
        onRpcRequest(createRequest('arkade_getAddress', { ...validParams, network: 'invalid' })),
    ...
    it('throws on incorrect signerPubkey length', async () => {
      await expect(
        onRpcRequest(
          createRequest('arkade_getAddress', { ...validParams, signerPubkey: 'a'.repeat(32) }),
        ),
    ...
    it('accepts numeric string for unilateralExitDelay', async () => {
      await expect(
        onRpcRequest(
          createRequest('arkade_getAddress', { ...validParams, unilateralExitDelay: '1024' }),
        ),
  ...
  describe('arkade_signPsbt', () => {
    const validPsbt = 'cHNidP8BAHECAAAAAe6F0YcN3vZJlvFP9kD8YgKK7y1AQBAAAAAAAP////8BAAAAAAAAAAAA';
    const validParams = {
      psbt: validPsbt,
      inputIndexes: [0],
    };
    ...
    it('returns valid base64 PSBT', async () => {
      const result = (await onRpcRequest(
        createRequest('arkade_signPsbt', validParams),
      )) as { psbt: string };
    ...
    it('throws on missing psbt parameter', async () => {
      await expect(
        onRpcRequest(createRequest('arkade_signPsbt', { inputIndexes: validParams.inputIndexes })),
      ).rejects.toThrow('Invalid psbt type');
    });

    it('throws on invalid psbt format (not base64)', async () => {
      await expect(
        onRpcRequest(createRequest('arkade_signPsbt', { ...validParams, psbt: 'not-base64!@#$' })),
    ...
    it('throws on empty inputIndexes array', async () => {
      await expect(
        onRpcRequest(createRequest('arkade_signPsbt', { ...validParams, inputIndexes: [] })),
      ).rejects.toThrow('inputIndexes cannot be empty');
    });

    it('throws on non-array inputIndexes', async () => {
      await expect(
        onRpcRequest(createRequest('arkade_signPsbt', { ...validParams, inputIndexes: 'not-array' })),
      ).rejects.toThrow('Invalid inputIndexes type: expected array');
    });

    it('throws on non-integer inputIndexes', async () => {
      await expect(
        onRpcRequest(createRequest('arkade_signPsbt', { ...validParams, inputIndexes: [0.5] })),
      ).rejects.toThrow('must be an integer');
    });

    it('throws on negative inputIndexes', async () => {
      await expect(
        onRpcRequest(createRequest('arkade_signPsbt', { ...validParams, inputIndexes: [-1] })),
      ).rejects.toThrow('must be non-negative');
    });

    it('accepts multiple inputIndexes', async () => {
      await expect(
        onRpcRequest(createRequest('arkade_signPsbt', { ...validParams, inputIndexes: [0, 1, 2] })),
      ).resolves.toBeDefined();
    });
  });
