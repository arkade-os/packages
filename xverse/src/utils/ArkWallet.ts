...
import { XverseIdentity } from './XverseIdentity';

export type SatsConnectNetwork = 'Mainnet' | 'Testnet' | 'Signet' | 'Regtest';

export interface ArkWalletConfig {
  arkServerUrl: string;
  esploraUrl: string;
...
export interface ArkWalletInfo {
  arkAddress: string;
...
export class ArkWallet {
  ...
  private identity: XverseIdentity | null = null;
  private config: ArkWalletConfig;

  constructor(config: ArkWalletConfig) {
    this.config = config;
    this.reset();
  }

  updateConfig(config: ArkWalletConfig) {
    this.config = config;
  }
  ...
  getIdentity(): XverseIdentity | null {
    return this.identity;
  }

  async connect(): Promise<ArkWalletInfo> {
    try {
      ...
      this.identity = new XverseIdentity(fullPubkey, signingAccount.address, request);
  ...
  async sendBitcoinAdvanced(recipient: string, amount: number) {
    ...
    const finalCheckpoints = await Promise.all(
      signedCheckpointTxs.map(async (checkpoint) => {
        const serverCheckpoint = Transaction.fromPSBT(
          scureBase.base64.decode(checkpoint)
        );
  ...
  async getBoardingUtxos() {
    ...
    try {
      ...
      throw new Error(
        `Failed to get boarding UTXOs: ${error?.message || 'Unknown error'}`
      );
    }
  }
